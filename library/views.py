...
class LoanViewSet(viewsets.ModelViewSet):
    ...
    def extend_loan_due_date(self, request, pk=None):
        ...
        if loan.due_date > timezone.now().date():
            ...
            try:
                loan.due_date = timezone.datetime.strptime(new_due_date, "%Y-%m-%d")
                loan.save()
                return Response(
                    {"status": "Due date extended succesfully"},
                    status=status.HTTP_200_OK,
                )
            except Exception:
                return Response(
                    {"status": "Invlid date format"}, status=status.HTTP_400_BAD_REQUEST
                )
        else:
            return Response(
                {
                    "status": "Due Date already passed, can not extend loan due date anymore."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
